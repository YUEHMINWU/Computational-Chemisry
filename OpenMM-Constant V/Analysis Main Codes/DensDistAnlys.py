...
class DDP:
    ...
    def f_g_plot(self, y_shift, V_drops_cat, colors, linestyles):
...
class DDP_oneside:
    ...
    def f_g_plot_oneside(self, y_nticks, y_text, y_shift, NTD_list, V_drops, colors, linestyles, fg_names):
        fig, ax = plt.subplots()
        ...
        plt.text(12.5, y_text, fg_names[0], fontsize=12)
        plt.text(12.5, y_text + y_shift, fg_names[1], fontsize=12)
...
class RPS:
    ...
    def plot_rdfs_12(self, dmax, ylim, edge, rdf, V_drops, colors, linestyles):
        ...
        for idx, rdf in enumerate(rdf):
            ...
            plt.plot(edge, rdf, label=labels, color=colors[idx], linestyle=linestyles[idx],lw=1.25)
