    ...
    def w_m_plots(self, NTCD_list, voltages, colors, linestyles, y_nticks):
        ...
        for k in range(len(voltages)):
            ...
            plt.plot(np.arange(0, len(NTCD_list[k])) * self.bin_width,
                     NTCD_list[k], label=labels, color=colors[k], linestyle=linestyles[k], lw=1.2)
...
class pospot:
    ...
    def plot_pospot(self, colors, linestyles):
        ...
        for i in range(len(self.rho_sys)):
            labels = f'0 / 0 V' if self.V_init[i] == 0 else f'-{self.V_init[i]} / {self.V_init[i]} V'
        ...
        plt.ylim(-self.V_init[-1], self.V_init[-1])
