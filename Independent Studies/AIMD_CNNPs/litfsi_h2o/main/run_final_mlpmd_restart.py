...
def read_xyz_manual(file):
    ...
    if natoms != 244:
        raise ValueError(f"Expected 3050 atoms, but found {natoms} in {file}")
...
def generate_lammps_input(
    ...
    # Determine the latest restart file and compute remaining steps
    restart_prefix = f"{label}.restart."
    restart_files = [
        f
        for f in os.listdir("../logs")
        if f.startswith(restart_prefix) and f[len(restart_prefix) :].isdigit()
    ]
    if restart_files:
        steps = [int(f[len(restart_prefix) :]) for f in restart_files]
        latest_step = max(steps)

    # Output file paths
    eq_pos_file = os.path.join(results_dir, f"{label}-eq-pos-re{latest_step}.xyz")
    prod_pos_file = os.path.join(results_dir, f"{label}-prod-pos-re{latest_step}.xyz")
    prod_vel_file = os.path.join(results_dir, f"{label}-prod-vel-re{latest_step}.xyz")
    prod_frc_file = os.path.join(results_dir, f"{label}-prod-frc-re{latest_step}.xyz")
...
restart         20000 {label}.restart


dump            1 all custom 20 {prod_pos_file} element xu yu zu
dump_modify     1 element {" ".join(atom_types)}
dump_modify     1 sort id

dump            2 all custom 20 {prod_vel_file} element vx vy vz
dump_modify     2 element {" ".join(atom_types)}
...
def run_mlpmd(
    ...
    atom_types=ATOM_TYPES,
    temperature=298.15,
    thermostat_damping=0.0,  # Damping time in fs
):
    ...
    if restart_files:
        steps = [int(f[len(restart_prefix) :]) for f in restart_files]
        latest_step = max(steps)
        remaining_steps = prod_steps - latest_step
        if remaining_steps <= 0:
            print("Simulation already completed or exceeded production steps.")
            return
        restart_file = f"{restart_prefix}{latest_step}"
    else:
        remaining_steps = prod_steps
        latest_step = initial_restart_step
        restart_file = f"{restart_prefix}{initial_restart_step}"

    # Check if the restart file exists
    restart_path = os.path.join(log_dir, restart_file)
    if not os.path.exists(restart_path):
        raise FileNotFoundError(f"Restart file not found: {restart_path}")

    print(f"Restarting from {restart_file} with {remaining_steps} steps remaining.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--xyz_file",
        default="init_structs/init_struct_0.extxyz",
    ...
    parser.add_argument(
        "--model_file",
        default="../results/allegro_model_output_primary_iter_0/deployed.nequip.pth",
